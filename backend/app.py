...
def search_creators():
    """Paginated, filtered creator search used by the Dashboard."""
    try:
        ...
        # Niche filter — resolve alias (Tech → technology)
        if niche and niche != 'all':
            resolved = NICHE_ALIASES.get(niche, niche)
            df = df[df['niche'].str.lower().str.contains(resolved, na=False)]

        # Text search across niche and creator_id
        if q:
            df = df[df['niche'].str.lower().str.contains(q, na=False)]
        ...
        for idx, row in enumerate(page_df.to_dict('records')):
            abs_idx = start + idx
            name = CREATOR_NAMES[int(row['creator_id']) % len(CREATOR_NAMES)]
            followers_val = int(row['followers'])
            followers_str = (
                f"{followers_val/1_000_000:.1f}M" if followers_val >= 1_000_000
                else f"{followers_val/1_000:.0f}K"
            )
            score = int(row.get('growth_score', 50) * 0.5 + row.get('authenticity_score', 50) * 0.5)
            tier  = 'S' if followers_val > 500_000 else 'A' if followers_val > 100_000 else 'B'
            results.append({
                'id':       int(row['creator_id']),
                'name':     name,
                'handle':   f"@{name.lower().replace(' ', '_')}",
                'cat':      str(row['niche']).title(),
                ...
                'score':     score,
                'tier':      tier,
                'av':        ''.join(p[0] for p in name.split()),
                'c1':        '#E1F5EE' if abs_idx % 2 == 0 else '#E6F1FB',
                'c2':        '#085041' if abs_idx % 2 == 0 else '#0C447C',
            })

        return jsonify({'results': results, 'total': total, 'page': page, 'pages': pages}), 200
